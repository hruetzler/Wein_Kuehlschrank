...
function parseJSON() {
    ...
    xhttp.open("GET", "/statusSwitch", true);
    xhttp.send();
}
...
function bearbeitenSp() {
    ...
    let xhr = new XMLHttpRequest();
    xhr.open("PATCH", "/datenAktualisieren", true);
...
function bearbeitenDe() {
    ...
    const datenString = JSON.stringify(daten);
    let xhr = new XMLHttpRequest();
    xhr.open("DELETE", "/datenDelete", true);
